<!doctype html>
<html>
<head>
<?php
 require($_SERVER['DOCUMENT_ROOT'] . "/Synergist/inc/head.html");
 ?></head>

<body>

<div class="container">
<?php
 require($_SERVER['DOCUMENT_ROOT'] . "/Synergist/inc/nav.html");
 ?>
  
  <article class="content">
    <h1>Bio</h1>
    <section>
      Synergist is a hard rock band from Toronto, Canada. Formed by longtime friends CutcH, Jon Lewis and Christopher Lyttle, 
      the&nbsp;band blends alternative, metal, prog and punk influences to create a unique style. Striving to find 
      a balance between songwriting and pushing themselves musically, Synergist utilizes each member's skills and tastes for a 
      melodic heavy&nbsp;sound. 
      <br />&nbsp;<br />
      2013 saw the release of the Synergist's self-titled debut album, which was written, recorded and produced independently by the band. 
      It&nbsp;is available for download at <a href="http://synergist.bandcamp.com">synergist.bandcamp.com</a>.
      <br>&nbsp;<br />
      <hr style="margin-top:15px;" />We are seeking a new bassist, and potentially other members.<br> Get in touch via email, 
      <a href="mailto:contact@synergistband.com">contact@synergistband.com</a>, or <a href="social.php">hit us up on social media!</a><hr style="margin-top:8px" /> 
      <br>&nbsp;
    </section>
  <!-- end .content --></article>
  <aside class="centered">
    <img src="images/bioPic.jpg" style="width:100%;">
  </aside>
<?php
 require($_SERVER['DOCUMENT_ROOT'] . "/Synergist/inc/footer.html");
 ?>
  <!-- end .container --></div>
</body>
</html>
